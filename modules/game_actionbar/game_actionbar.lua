...
function onMultiUseCooldown(multiUseCooldown)
    ...
    for _, actionbar in pairs(bars) do
        if actionbar.tabBar then
            for _, button in pairs(actionbar.tabBar:getChildren()) do
                updateButtonState(button)
                if multiUseCooldown and button.item then
                    if isDarkDungeonsPotionButton(button) then
                        updateCooldown(button, multiUseCooldown)
                    elseif button.cache and button.cache.itemId then
                        local item = button.item:getItem()
                        if item and item:isMultiUse() then
                            local marketData = item:getMarketData()
                            local category = marketData and marketData.category
                            local marketArray = {MarketCategory.Potions, MarketCategory.Runes, MarketCategory.Tools}
                            if table.contains(marketArray, category) then
                                updateCooldown(button, multiUseCooldown)
                            end
                        end
                    end
