...
-- Dark Dungeons: client-side visual cooldown for shared health/mana potion cooldown.
...
local function getDarkDungeonsButtonItemId(button)
    ...
    if button.cache and button.cache.itemId then
        return button.cache.itemId
    end

    if button.item and button.item.getItemId then
...
local function applyDarkDungeonsPotionCooldown(button, timeMs)
    ...
    updateCooldown(button, timeMs)
    button.cache.removeCooldownEvent = scheduleEvent(function()
        removeCooldown(button)
    end, timeMs)
end

local function refreshDarkDungeonsPotionButtons(timeMs)
    local bars = actionBars or activeActionBars
    if not bars then
        return
    end
    ...
    for _, actionbar in pairs(bars) do
        if actionbar.tabBar then
            for _, actionButton in pairs(actionbar.tabBar:getChildren()) do
                if isDarkDungeonsPotionItem(getDarkDungeonsButtonItemId(actionButton)) then
                    applyDarkDungeonsPotionCooldown(actionButton, timeMs)
                end
            end
        end
    end
...
local function startDarkDungeonsPotionCooldown(button)
    if not isDarkDungeonsPotionItem(getDarkDungeonsButtonItemId(button)) then
        return false
    end

    local now = g_clock.millis()
    if ddPotionCooldownUntil > now then
        local remainTime = ddPotionCooldownUntil - now
        applyDarkDungeonsPotionCooldown(button, remainTime)
        refreshDarkDungeonsPotionButtons(remainTime)
        return true
    end

    ddPotionCooldownUntil = now + DD_POTION_COOLDOWN_MS
    applyDarkDungeonsPotionCooldown(button, DD_POTION_COOLDOWN_MS)
    refreshDarkDungeonsPotionButtons(DD_POTION_COOLDOWN_MS)
    return false
end
...
function onExecuteAction(button, isPress)
    ...
    if action == UseTypes["Use"] and button.item then
        if startDarkDungeonsPotionCooldown(button) then
            return true
        end
